...
export default @injectIntl class NavHeader extends Component {
  toggleLan = () => {
...
/*  goIndex = () =>  {
  ...
  render() {
    ...
    if (pgindex||(pgindex === "" && (status === "/"||status === "/index.html"))) {
          this.state.btnFlag = false
    }else {
      this.state.btnFlag = true
    }
              ...
              <div className={styles.nav_bar}>
                <ul>
                  ...
                  {HEADER_NAV.map(({ to, txt, items }, i) => (    
                      <li style={{backgroundColor: (to === status) ? '#024753': '' }} key={i + '-' + txt}>
                          <Link to={to}>{msg(txt)}
                          {/* {items && <div className={styles.hid}>
                            <ul>{items.map(({ to, txt, src }) => <li key={txt}>
                                <Link to={to}>
                                  {src ? <><img src={src} />
                                  <p>{msg(txt)}</p>
                                  </> : msg(txt)}
